...
9. **Winners are confirmed, not crowned.** Choosing the best of several arms across many metrics is a multiple-comparisons problem: enough cells, and one delta clears the margin by luck. A round winner that drives the user's decision is confirmed on data the matrix has not tuned against: the eval set's held-out slice (references/case-guide.md) or fresh cases written to the same case guide, same rules, same margins. Internal rounds that only pick the next perturbation skip the confirmation; the decision itself does not.
10. **Judge-graded deltas are compared pairwise.** When a judge-graded metric feeds an arm delta, the plan compares the arms' outputs per case with the pairwise mode and both-orderings rule (references/judge-eval.md design step 5), not two independent absolute scores. The judge's identity, mode, and validation agreement are identical in every arm and are reported with the delta table.
...
Judging: judge-graded metrics compared pairwise per references/judge-eval.md; judge identity, mode, and validation agreement identical in every arm and reported with the delta table
Confirmation: <the decision-driving winner is confirmed on the held-out slice or fresh cases per rule 9, or "not applicable: single-round matrix">
